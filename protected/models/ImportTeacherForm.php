...
class ImportTeacherForm extends CFormModel
{
	public $file;
	public $filePath;
	public $fileName = 'sample-import-teacher.xlsx';
	...
	public function save() {
		foreach($this->readData() as $row):
			...
			$model->code		= Teacher::model()->generateCode();
			$check = Teacher::model()->findByNik($row[0]);
			if($check) {
                $this->error++;
			...
			$model->marital_status = $row[9];
			$model->previous_education_id = $row[10];
			$model->instance_previous_education = $row[11];
			$model->graduation_year = $row[12];
			$model->teacher_status = $row[13];
			$model->status      = $row[14];
